...
def primakoff_scattering_xs(energy, z, ma, g):  # Avignone '87
    if energy < ma:
        return 0
    beta = max(np.sqrt(energy ** 2 - ma ** 2)/energy, 0.95)
    prefactor = (1 / 137 / 2) * (g * z) ** 2
    def xs(beta):
        return max((1 / beta) * ((1 + beta ** 2) / (2 * beta) * np.log((1 + beta) / (1 - beta)) - 1), 0)
    
    return prefactor * xs(beta)

def primakoff_scattering_diffxs(theta, ea, g, ma, z, r0):
    if ea < ma:
        return 0.0
    prefactor = (g * z)**2 / (2*137)
    #q2 = -2 * ea**2 * (1-cos(theta))
...
def primakoff_scattering_xs_complete(ea, g, ma, z, r0):
...
class PrimakoffAxionFromBeam:
    def __init__(self, photon_rates=[1.,1.,0.], target_z=90, target_photon_cross=15e-24,
                 detector_distance=4., detector_length=0.2, detector_area=0.04, axion_mass=0.1,
                 axion_coupling=1e-3, nsamples=10000):
        ...
        self.thetas = np.random.uniform(0.0, pi, nsamples)
    ...
    def simulate_single(self, photon):
        ...
        integral = 4*pi*np.sum(convolution(self.thetas, self.phis))/len(self.thetas)
    ...
    def simulate(self, nsamples=10):
        ...
        print("Running with N_CPUs = ", max(1, multi.cpu_count()-1))
    ...
    def propagate(self):
        ...
        decay_prob = np.array([fsub(1,mp.exp(-self.det_length / meter_by_mev / v_a[i] / tau[i])) \
                      for i in range(len(v_a))])
        self.decay_axion_weight = g**2 * wgt * surv_prob * decay_prob
        self.scatter_axion_weight = g**2 * wgt * surv_prob
        ...
        for i in range(len(self.scatter_axion_weight)):
            if self.axion_energy[i] >= threshold:
                res += self.scatter_axion_weight[i] \
                    * primakoff_scattering_xs_CAF(self.axion_energy[i], self.axion_coupling, 
